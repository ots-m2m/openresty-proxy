...
RUN \
  ...
  | tar xz && \
  ./configure --with-pcre-jit --with-ipv6 --with-http_auth_request_module --add-module=./ngx_http_substitutions_filter_module-$NGX_SUBSTITUTIONS_FILTER_VERSION && \
  make && \
  make install && \
  make clean && \
  cd .. && \
